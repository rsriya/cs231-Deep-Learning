...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        factor = torch.pow(10000,-torch.arange(0, embed_dim, 2)/embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query_batch = query.view(N, S, self.n_head, self.head_dim).transpose(1,2) #N, H, S, E/H
        key_batch = key.view(N, T, self.n_head, self.head_dim).permute(0,2,3,1) #N, H, E/H, T
        
        attn_scores = torch.matmul(query_batch, key_batch)/math.sqrt(self.head_dim) #N, H, S, T
        if attn_mask is not None:
            attn_scores = attn_scores.masked_fill(attn_mask == 0, float('-inf'))
        attn_weights = F.softmax(attn_scores, dim=-1)  # (N, H, T, T)
        attn_weights = self.attn_drop(attn_weights)
        
        value_batch = value.view(N, T, self.n_head, self.head_dim).transpose(1,2) # N, H, T, E/H
        output_batch = torch.matmul(attn_weights, value_batch) #N, H, S, E/H
        output_batch = output_batch.transpose(1,2) #N, S, H, E/H
        output = output_batch.reshape(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        x_divided = torch.reshape(x, (N, C, self.img_size // self.patch_size, self.patch_size, self.img_size // self.patch_size, self.patch_size))
        x_divided = torch.permute(x_divided, (0, 2, 4, 1, 3, 5))
        x_divided = torch.reshape(x_divided, (N, self.num_patches, self.patch_dim))
        out = self.proj(x_divided)               
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
