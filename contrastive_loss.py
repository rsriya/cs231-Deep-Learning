...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        pos_pair_sim = np.exp(sim(z_k, z_k_N)/tau)
        neg_pair_sim_k = [np.exp(sim(z_k, out[i])/tau) for i in range(2*N) if i != k]
        neg_pair_sim_k_N = [np.exp(sim(z_k_N, out[i])/tau) for i in range(2*N) if i != k+N]        
        total_loss = total_loss-np.log(pos_pair_sim/np.sum(neg_pair_sim_k))-np.log(pos_pair_sim/np.sum(neg_pair_sim_k_N)) 
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    pos_pairs = (out_left*out_right/(torch.linalg.norm(out_left, dim=1, keepdim=True)*torch.linalg.norm(out_right, dim=1, keepdim=True))).sum(dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    sim_matrix = out/torch.linalg.norm(out, dim=1, keepdim=True) @ (out/torch.linalg.norm(out, dim=1, keepdim=True)).T
